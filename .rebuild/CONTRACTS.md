...
export const YOU_SLOW_KEYS = ["quests","flags","choices","party","items","claims","history","respawn","wallet","dialogue","notices","kitReadout"]; // ride the slow frame as youSlow; fast.you carries the rest (the open dialogue and the notices change only when something lands on the viewer, and an action sends the slow frame at once)
...
export class SlowTracker { fresh(viewer); rosterDue(viewer, fast); diff(viewer, slow, cache?): SlowFrame | null; forget(viewer) } // a section that is the object it sent last is unchanged without a stringify; youSlow likewise by its records' identity; a roster whose every entry is the object seen last is unchanged without a rebuild
