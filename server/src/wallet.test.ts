...
import { addressOfPublicKey, challengeMessage, isAddress, parseHolders, personalMessageHash, recoverAddress } from "./wallet";
...
describe("wallet proofs", () => {
  ...
  it("recovers the signer of a personal_sign over the challenge and rejects everything else", () => {
    const priv = key(7);
    const address = addressOfPublicKey(secp256k1.getPublicKey(priv, false));
    const message = challengeMessage("nonce-1", 1_700_000_000_000);
    ...
    expect(recoverAddress(challengeMessage("nonce-2", 1_700_000_000_000), sig)).not.toBe(address);
