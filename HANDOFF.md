...
- Validation: client and server type-checks and production build passed; 359 tests covered across the full suite and corrected campaign/PvP/server rerun. World, intake, memorial and two-Angel PvP smoke checks passed against local Wrangler. Added 12 simulation regressions and a public-protocol flag persistence test. Updated earlier combat fixtures to declare participant flags; insurance/print-loss checks now use clerk deaths so they do not rely on attacking unflagged people. The two-Angel smoke script walks the real corridor, checks no/single/both flags, deals one ordinary hit, then tests truce and saved reconnect without killing either participant. It also passed on the deployed Worker, alongside both memorial paths; the served release and bundle matched GitHub.

## Stage 273 (landed)
- Corrected alarm-driven simulation drift. Callbacks formerly advanced exactly 50ms regardless of delivery/storage overhead; two live relief-shift checks timed out and one sample ran about 0.6 simulation seconds per wall second. The clock now accumulates elapsed server time, carries fractional frames, and runs fixed 50ms simulation steps.
- Each update catches up at most 250ms. Long outages are discarded instead of replaying minutes of movement or combat. Duplicate/early callbacks do not create time. Idle worlds stop scheduling; resumed worlds begin a fresh clock rather than progressing offline.
- Elapsed time is settled before new client input, preventing a fresh dodge or direction from being applied retroactively across a delayed frame. Clients cannot supply elapsed time. Stale intent expiry remains server-controlled.
- Constructor recovery preserves an existing alarm, following [Cloudflare's alarm lifecycle guidance](https://developers.cloudflare.com/durable-objects/api/alarms/). Scheduling from callback start also avoids adding checkpoint overhead to every interval.
- Validation: client/server type-checks and production build passed. All 368 tests passed, including delayed/duplicate alarm delivery, bounded outage recovery, preserved alarms, idle stop, input ordering and clock jitter. `scripts/smoke-clock.mjs` measures six seconds of actual server time against wall time; its drift bound is 15 percent. Local Wrangler measured 6.05 simulation seconds over 6.01 wall seconds (ratio 1.006); the clock, world, intake, memorial and PvP smoke checks all passed. This measures simulation pacing, not rendering FPS.

## Stage 274 (do this next)
...
- Server simulation drift was corrected in Stage 273. Retain the live clock measurement alongside the gameplay smoke tests; it does not establish visual frame pacing. Keep long-outage catch-up bounded.
...
- Validation: 374 tests pass; client/server type-checks and production build pass. New public WebSocket campaign smoke walks Movement I, guest lock, mock Angel, refusal/garden, Movement III, three organs and Ord, verifies saved reconnect, then denies organ shortcuts to another new arrival. Local and live campaign runs pass. Live movement, timed dodge, saved reconnect and single-tab ownership also pass. The release check caught two nonexistent journal filenames; corrected them to the existing `organ-foundry-dark.jpg` and `organ-cable-dark.jpg`. All 20 journal asset references exist; corrected assets are included in the release checks.
