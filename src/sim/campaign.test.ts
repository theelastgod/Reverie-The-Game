...
describe("File — people", () => {
  it("names filing as people after claims; TAKE stays disarmed; guests cannot", () => {
    const w = emptyWorld();
    w.claimsPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), claimsPeople: true },
      x: CLAIMS_DESK.x,
      y: CLAIMS_DESK.y,
    });
    const named = applyRead(w, "a", CLAIMS_DESK.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(FILE_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_FILE_PEOPLE);
    expect(p.beats.filePeople).toBe(true);
    expect(named.filePeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "file-people")?.name).toBe("File — people");
    expect(named.signs.find((s) => s.id === "file-people")?.title).toBe(FILE_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("TAKE stays disarmed");
    expect(p.heard).not.toMatch(/heidegger|midgar/i);
    expect(damageFor(p)).toBe(damageFor(spawnGuest("g")));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyFilePeople(named, "a").players.get("a")?.heard).toBe(FILE_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: CLAIMS_DESK.x, y: CLAIMS_DESK.y });
    expect(applyFilePeople(early, "a").players.get("a")?.heard).toBe(FILE_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.claimsPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: CLAIMS_DESK.x, y: CLAIMS_DESK.y, locked: true });
    expect(applyFilePeople(gWorld, "g").players.get("g")?.heard).toBe(FILE_PEOPLE_SPECTATOR);
    expect(gWorld.filePeopleHeld).toBe(false);
  });
});

describe("TAKE — people", () => {
  it("names TAKE as people after filing; TAKE stays disarmed; guests cannot", () => {
    const w = emptyWorld();
    w.filePeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), filePeople: true },
      x: CLAIMS_DESK.x,
      y: CLAIMS_DESK.y,
    });
    const named = applyRead(w, "a", CLAIMS_DESK.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(TAKE_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_TAKE_PEOPLE);
    expect(p.beats.takePeople).toBe(true);
    expect(named.takePeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "take-people")?.name).toBe("TAKE — people");
    expect(named.signs.find((s) => s.id === "take-people")?.title).toBe(TAKE_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("TAKE stays disarmed");
    expect(p.heard).not.toMatch(/heidegger|midgar/i);
    expect(damageFor(p)).toBe(damageFor(spawnGuest("g")));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyTakePeople(named, "a").players.get("a")?.heard).toBe(TAKE_PEOPLE_HELD);
    const due = { ...named, now: 10 };
    due.players.set("a", {
      ...due.players.get("a")!,
      claims: [{ id: "c1", amount: 12, created: 0, readyAt: 1 }],
    });
    expect(applyDesk(due, "a", "take").players.get("a")?.heard).toBe(DESK_DISARMED);
    expect(applyDesk(due, "a", "take").players.get("a")?.bestand).toBe(0);
    expect(CLAIMS_ARMED).toBe(false);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: CLAIMS_DESK.x, y: CLAIMS_DESK.y });
    expect(applyTakePeople(early, "a").players.get("a")?.heard).toBe(TAKE_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.filePeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: CLAIMS_DESK.x, y: CLAIMS_DESK.y, locked: true });
...
describe("Unflag — people", () => {
  it("names unflag as people after spoils; unflag still opts out; guests cannot", () => {
    const w = emptyWorld();
    w.spoilsPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), spoilsPeople: true },
      x: WET_GRID.x,
      y: WET_GRID.y,
    });
    const named = applyRead(w, "a", WET_GRID.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(UNFLAG_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_UNFLAG_PEOPLE);
    expect(p.beats.unflagPeople).toBe(true);
    expect(named.unflagPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "unflag-people")?.name).toBe("Unflag — people");
    expect(named.signs.find((s) => s.id === "unflag-people")?.title).toBe(UNFLAG_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("Unflag still opts out");
    expect(p.heard).not.toMatch(/heidegger|midgar/i);
    expect(damageFor(p)).toBe(damageFor(spawnGuest("g")));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyUnflagPeople(named, "a").players.get("a")?.heard).toBe(UNFLAG_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: WET_GRID.x, y: WET_GRID.y });
    expect(applyUnflagPeople(early, "a").players.get("a")?.heard).toBe(UNFLAG_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.spoilsPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: WET_GRID.x, y: WET_GRID.y, locked: true });
...
describe("Kit — people", () => {
  it("names the kit as people after grief; same number; guests cannot", () => {
    const w = emptyWorld();
    w.griefPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), griefPeople: true },
      x: GUEST_ARENA.x,
      y: GUEST_ARENA.y,
    });
    const named = applyRead(w, "a", GUEST_ARENA.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(KIT_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_KIT_PEOPLE);
    expect(p.beats.kitPeople).toBe(true);
    expect(named.kitPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "kit-people")?.name).toBe("Kit — people");
    expect(named.signs.find((s) => s.id === "kit-people")?.title).toBe(KIT_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("Same number");
    expect(p.heard).not.toMatch(/heidegger|midgar/i);
    expect(damageFor(p)).toBe(damageFor(spawnGuest("g")));
    expect(damageFor(p)).toBe(STRIKE_DAMAGE);
    expect(guestCanClaim(p)).toBe(false);
    expect(applyKitPeople(named, "a").players.get("a")?.heard).toBe(KIT_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: GUEST_ARENA.x, y: GUEST_ARENA.y });
    expect(applyKitPeople(early, "a").players.get("a")?.heard).toBe(KIT_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.griefPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: GUEST_ARENA.x, y: GUEST_ARENA.y, locked: true });
...
describe("Geared — people", () => {
  it("names geared graves as people after the dummy; geared still crack; guests cannot", () => {
    const w = emptyWorld();
    w.dummyPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), dummyPeople: true },
      x: WET_GRID.x,
      y: WET_GRID.y,
    });
    const named = applyRead(w, "a", WET_GRID.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(GEARED_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_GEARED_PEOPLE);
    expect(p.beats.gearedPeople).toBe(true);
    expect(named.gearedPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "geared-people")?.name).toBe("Geared — people");
    expect(named.signs.find((s) => s.id === "geared-people")?.title).toBe(GEARED_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("Geared graves still crack");
    expect(p.heard).not.toMatch(/heidegger|midgar/i);
    expect(damageFor(p)).toBe(damageFor(spawnGuest("g")));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyGearedPeople(named, "a").players.get("a")?.heard).toBe(GEARED_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: WET_GRID.x, y: WET_GRID.y });
    expect(applyGearedPeople(early, "a").players.get("a")?.heard).toBe(GEARED_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.dummyPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: WET_GRID.x, y: WET_GRID.y, locked: true });
...
describe("Band — people", () => {
  it("names the band as people after serials; same skill different serials same number; guests cannot", () => {
    const w = emptyWorld();
    w.serialPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), serialPeople: true },
      x: WET_GRID.x,
      y: WET_GRID.y,
    });
    const named = applyRead(w, "a", WET_GRID.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(BAND_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_BAND_PEOPLE);
    expect(p.beats.bandPeople).toBe(true);
    expect(named.bandPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "band-people")?.name).toBe("Band — people");
    expect(named.signs.find((s) => s.id === "band-people")?.title).toBe(BAND_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("different serials");
    expect(p.heard).not.toMatch(/heidegger|midgar/i);
    const other = { ...spawnGuest("b"), guest: false, serial: 2222 };
    expect(damageFor(p)).toBe(damageFor(other));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyBandPeople(named, "a").players.get("a")?.heard).toBe(BAND_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: WET_GRID.x, y: WET_GRID.y });
    expect(applyBandPeople(early, "a").players.get("a")?.heard).toBe(BAND_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.serialPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: WET_GRID.x, y: WET_GRID.y, locked: true });
...
describe("Stay — people", () => {
  it("names staying as people after waiting; Nara still stays at the hole; guests cannot", () => {
    const w = emptyWorld();
    w.waitPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), waitPeople: true },
      x: WET_GRID.x,
      y: WET_GRID.y,
    });
    const named = applyRead(w, "a", WET_GRID.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(STAY_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_STAY_PEOPLE);
    expect(p.beats.stayPeople).toBe(true);
    expect(named.stayPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "stay-people")?.name).toBe("Stay — people");
    expect(named.signs.find((s) => s.id === "stay-people")?.title).toBe(STAY_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("Nara still stays at the hole");
    expect(p.heard).toContain("Absence still waits");
    expect(p.heard).not.toMatch(/heidegger|midgar|meltdown/i);
    const other = { ...spawnGuest("b"), guest: false, serial: 2222 };
    expect(damageFor(p)).toBe(damageFor(other));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyStayPeople(named, "a").players.get("a")?.heard).toBe(STAY_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: WET_GRID.x, y: WET_GRID.y });
    expect(applyStayPeople(early, "a").players.get("a")?.heard).toBe(STAY_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.waitPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: WET_GRID.x, y: WET_GRID.y, locked: true });
    expect(applyStayPeople(gWorld, "g").players.get("g")?.heard).toBe(STAY_PEOPLE_SPECTATOR);
    expect(gWorld.stayPeopleHeld).toBe(false);
  });
});

describe("Willing — people", () => {
  it("names willingness as people after staying; Appearance still needs the party willing; guests cannot", () => {
    const w = emptyWorld();
    w.stayPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), stayPeople: true },
      x: WET_GRID.x,
      y: WET_GRID.y,
    });
    const named = applyRead(w, "a", WET_GRID.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(WILLING_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_WILLING_PEOPLE);
    expect(p.beats.willingPeople).toBe(true);
    expect(named.willingPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "willing-people")?.name).toBe("Willing — people");
    expect(named.signs.find((s) => s.id === "willing-people")?.title).toBe(WILLING_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("Appearance still needs the party willing");
    expect(p.heard).not.toMatch(/heidegger|midgar|meltdown/i);
    const other = { ...spawnGuest("b"), guest: false, serial: 2222 };
    expect(damageFor(p)).toBe(damageFor(other));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyWillingPeople(named, "a").players.get("a")?.heard).toBe(WILLING_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: WET_GRID.x, y: WET_GRID.y });
    expect(applyWillingPeople(early, "a").players.get("a")?.heard).toBe(WILLING_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.stayPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: WET_GRID.x, y: WET_GRID.y, locked: true });
    expect(applyWillingPeople(gWorld, "g").players.get("g")?.heard).toBe(WILLING_PEOPLE_SPECTATOR);
    expect(gWorld.willingPeopleHeld).toBe(false);
  });
});

describe("Empty — people", () => {
  it("names the empty party as people after willingness; walked party makes Passing absence; guests cannot", () => {
    const w = emptyWorld();
    w.willingPeopleHeld = true;
    w.players.set("a", {
      ...spawnGuest("a"),
      guest: false,
      serial: TEST_SERIAL,
      beats: { ...emptyBeats(), willingPeople: true },
      x: WET_GRID.x,
      y: WET_GRID.y,
    });
    const named = applyRead(w, "a", WET_GRID.id);
    const p = named.players.get("a")!;
    expect(p.heard).toBe(EMPTY_PEOPLE_COPY);
    expect(p.wink).toBe(WINK_EMPTY_PEOPLE);
    expect(p.beats.emptyPeople).toBe(true);
    expect(named.emptyPeopleHeld).toBe(true);
    expect(named.pois.find((poi) => poi.kind === "empty-people")?.name).toBe("Empty — people");
    expect(named.signs.find((s) => s.id === "empty-people")?.title).toBe(EMPTY_PEOPLE_PLAQUE.title);
    expect(p.heard).toContain("If Nara, Ord, or Quill walked");
    expect(p.heard).toContain("Passing is absence");
    expect(p.heard).not.toMatch(/heidegger|midgar|meltdown/i);
    const other = { ...spawnGuest("b"), guest: false, serial: 2222 };
    expect(damageFor(p)).toBe(damageFor(other));
    expect(guestCanClaim(p)).toBe(false);
    expect(applyEmptyPeople(named, "a").players.get("a")?.heard).toBe(EMPTY_PEOPLE_HELD);

    const early = emptyWorld();
    early.players.set("a", { ...spawnGuest("a"), guest: false, x: WET_GRID.x, y: WET_GRID.y });
    expect(applyEmptyPeople(early, "a").players.get("a")?.heard).toBe(EMPTY_PEOPLE_NEED);

    const gWorld = emptyWorld();
    gWorld.willingPeopleHeld = true;
    gWorld.players.set("g", { ...spawnGuest("g"), x: WET_GRID.x, y: WET_GRID.y, locked: true });
